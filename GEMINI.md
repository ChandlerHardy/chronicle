# Chronicle - Gemini Assistant Guide

> **Purpose**: This document tells Gemini assistants how to use Chronicle effectively.
...
```python
mcp__chronicle__get_sessions(limit=10, tool="gemini-cli", repo_path="/path", days=7)
mcp__chronicle__get_session_summary(session_id=16)
...
```
1. User runs: chronicle start gemini
   ↓
...
# Spawns: script -q ~/.ai-session/sessions/session_N.log gemini
...
If user is already in a Gemini session and asks about Chronicle:
- This session is **NOT being tracked** (unless started with `chronicle start gemini`)
- Suggest they exit and restart with `chronicle start gemini` for tracking
...
**Fix**: User must restart Gemini after editing `~/.mcp.json`
...
- `chronicle-skills/` - Gemini Skills definitions
