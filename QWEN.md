# Chronicle - Qwen Assistant Guide

> **Purpose**: This document tells Qwen assistants how to use Chronicle effectively.
...
```python
mcp__chronicle__get_sessions(limit=10, tool="qwen-cli", repo_path="/path", days=7)
mcp__chronicle__get_session_summary(session_id=16)
...
```
1. User runs: chronicle start qwen
   ↓
...
```bash
chronicle start qwen
# Spawns: script -q ~/.ai-session/sessions/session_N.log qwen
...
If user is already in a Qwen session and asks about Chronicle:
- This session is **NOT being tracked** (unless started with `chronicle start qwen`)
- Suggest they exit and restart with `chronicle start qwen` for tracking
...
### Qwen API Key (required for summarization):
```bash
chronicle config ai.qwen_api_key YOUR_KEY
# Or: export QWEN_API_KEY=...
```

### Important Settings:
- `ai.default_model` → Qwen model (default: `qwen-max` or `qwen-plus`)
- `ai.qwen_api_key` → API key for summarization
...
**Fix**: User must restart Qwen after editing `~/.mcp.json`
...
- `backend/services/summarizer.py` - Qwen integration, chunking logic
